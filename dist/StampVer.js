...
`);
    ...
    const now = _momentTimezone2.default.tz((0, _momentTimezone2.default)(), data.tz);
    ...
    for (let filename of data.filenames) {
      ...
      for (let fileType of data.fileTypes) {
        if (!(0, _minimatch2.default)(filename, fileType.glob)) {
          continue;
        }
        ...
        if (fileType.write) {
          ...
          if (args.update) {
            await _util2.default.promisify(_fs2.default.writeFile)(filename, StampVer.replaceTags(fileType.write, data.tags));
          }
        } else {
